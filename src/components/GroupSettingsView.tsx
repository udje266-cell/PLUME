          ...
          <div className="w-full bg-white dark:bg-[#0E0E14] rounded-t-3xl p-4 animate-slide-up" onClick={(e) => e.stopPropagation()} style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}>
            ...
            {(() => {
              ...
              if (m.userId === currentUser.id) {
                ...
                // Changer un role est reserve aux ADMINS (le serveur refuse a un
                // simple moderateur) : on ne propose donc ces actions qu'aux admins.
                if (isAdminPlus && m.role !== 'moderator' && m.role !== 'admin') items.push(<SheetItem key="mkmod" icon={<Shield className="w-4 h-4" />} label="Nommer modérateur" onClick={() => setRole(m.userId, 'moderator')} />);
                if (isAdminPlus && m.role === 'moderator') items.push(<SheetItem key="rmmod" icon={<UserIcon className="w-4 h-4" />} label="Retirer modérateur" onClick={() => setRole(m.userId, 'member')} />);
